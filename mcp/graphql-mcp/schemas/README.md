...
2.  The MCP Server enumerates this directory on `resources/list`.
3.  Each file is exposed as an MCP Resource with the URI: `graphql://local/<filename>`
    (read via `resources/read`). Names are path-traversal safe (basename only).
...
Example schema included: `schema.docs.graphql` (GitHub GraphQL API), readable via
`graphql://local/schema.docs.graphql`.
